...
### 1. `init_deep`: 지식 베이스 압축 및 상태 복구 🌟
프로젝트의 방대한 파일들을 AI가 한눈에 이해할 수 있도록 **"고밀도 지식 베이스(AGENTS.md)"**로 압축하고, 현재 작업 진행 상태를 동기화합니다.

*   **기본 개념**: 정적 지식(코드 구조)과 동적 상태(진행 상황)를 결합하여 `AGENTS.md`라는 **단일 진실 공급원(SSOT)**을 생성합니다.
*   **주요 시나리오**:
    *   **세션 복구 (Session Resume)**: 컴퓨터가 꺼졌거나 새 세션을 시작했을 때, `.omg/runtime_state.json`을 읽어 이전의 기억(마지막 작업 단계, 성공 여부 등)을 즉시 복구합니다.
    *   **컨텍스트 리프레시**: 대규모 수정 후 에이전트의 환각(Hallucination)을 방지하기 위해 최신 코드로 지식 베이스를 갱신합니다.
*   **명령어 옵션**:
    *   `init_deep 1`: 표면 분석 (초기 진입, 폴더 구조 파악)
    *   `init_deep 2`: 중층 분석 (주요 모듈 인터페이스 및 API 명세 파악)
    *   `init_deep 3`: 심층 분석 (복잡한 비즈니스 로직 및 알고리즘 분석)

### 2. `harness`: 결정론적 품질 게이트 (v3.2) 🛡️
단순 실행을 넘어 정적 분석, 무결성 검증, 상태 보고를 수행하는 통합 감사 엔진입니다.

*   **동작 매커니즘 (3-Layer Gate)**:
    1.  **L0 (Static)**: `ruff`, `tsc`를 이용해 문법과 타입을 강제합니다.
    2.  **L1 (Structural)**: `Hash-Anchored Edit` 시스템을 통해 기존 코드의 해시와 대조하여 무단 변경이나 위치 이탈을 차단합니다.
    3.  **L2 (Functional)**: 실제 런타임에서 기대하는 로그 패턴이 출력되는지 확인합니다.
*   **상태 자동 보고**: 검증 성공 시 `taskId`별로 체크포인트를 생성하여 `StateManager`에 실시간으로 성공을 기록합니다.

### 3. `State Manager`: 세션 연속성 엔진 🧠
에이전트의 바깥에서 '외장형 뇌' 역할을 수행하며 모든 활동을 기록합니다.

*   **Durable State Store**: `.omg/runtime_state.json`에 현재 단계(Phase), 작업 큐(Task Queue), 개별 작업 결과가 저장됩니다.
*   **Deterministic Replay**: 세션 재시작 시 이미 완료된 작업(COMPLETED)은 다시 수행하지 않고 다음 단계로 즉시 점프할 수 있는 근거를 제공합니다.
